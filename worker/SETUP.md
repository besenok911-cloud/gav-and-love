...
> **Секрети й змінні, без яких салон не працює.** У `wrangler.toml` — `ALLOW_ORIGIN` (адреса
> сайту, з якої дозволено звертатися до воркера) і `SITE_URL` (адреса сайту в посиланнях бота).
> Обидві треба поміняти під новий салон, інакше браузер упреться в CORS, а бот слатиме клієнтів
> на чужий сайт. Секретами (`wrangler secret put`): `ADMIN_TOKEN` — аварійний пароль власника
> до CRM, єдиний спосіб увійти в порожню базу; `PLATFORM_TOKEN` — ключ оператора до нічних
> дампів, власниці салону не передається.
>
> **Перший запуск нового салону.** База створюється з `worker/schema.sql` (повна схема, 16 таблиць),
> і вона порожня навмисно: майстрів і прайс салон заводить у CRM під себе. Доки в таблиці `masters`
> немає жодного рядка, форма запису не пропонує часу — це правильно, ненастроєний салон не повинен
> приймати записи. Раніше порожня база підставляла майстрів і прайс GAV&LOVE.
...
> Без календаря салон працює: зайнятий час рахується з власної таблиці записів. Календар потрібен
> тоді, коли майстри хочуть бачити день у своєму телефоні та вручну блокувати години.
